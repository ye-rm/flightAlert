...
1. 下载发布页中最新的 `FlightAlert.exe`（或对应版本提供的 `.exe`）文件
2. 双击运行程序
3. 在"配置设置"标签页中输入监控参数
4. 点击"保存配置"按钮保存设置
...
- `dateToGo`：需要监控的出发日期（日期格式为 `YYYYMMDD`，如 `20251105`）。
...
- `SCKEY`：`pushplus` 的 token，详见[pushplus 文档](https://www.pushplus.plus/doc/)获取方法；留空时仍可监控价格，但不会发送微信通知。
...
- **PushPlus 推送令牌**：`pushplus` 的 token，用于发送微信通知；界面会遮蔽输入内容，但这只防止屏幕旁观，不代表本地配置文件已加密。
...
1. 点击"开始监控"按钮开始价格监控
2. "当前状态"区域显示监控路线、监控日期、通知是否启用、上次检查和下次检查/重试倒计时
3. "当前价格"区域显示最新检测到的价格；尚未获取或请求失败时会显示明确提示
4. "活动日志"区域显示更详细的程序运行状态和价格变化记录
5. 需要停止监控时，点击"停止监控"按钮
